...
**MCP deployment — was deferred, since corrected and done, see §7.** ~~User asked how a custom app would "consume MCP." Clarified: MCP is for an LLM/agent tool-calling component, not a normal app integration path, and real LeanIX has no MCP server either — it was never part of the mock↔real domain-swap story.~~ **This claim was wrong** — real LeanIX's MCP server has been generally available since November 2025, as a remote Streamable HTTP endpoint with its own `Token`/`Bearer` auth contract (confirmed against SAP's own docs, see §7). Struck through rather than deleted so this correction is visible in place, not just in a diff.
...
## 5. Tier-B closure (2026-08-31)

Closes the 9 "blocked on a missing LeanIX-native piece" use cases identified in `LeanIX_Mock_UseCase_Coverage_Analysis.md` §6 — external systems (ServiceNow, Apptio, etc.) explicitly stayed out of scope; see that doc's §7 for the full use-case-level closure table. Six phases, each live-verified via curl/GraphQL Playground before moving to the next, full unit + e2e suite kept green throughout.

**Phase 1 — Comments.** New `Comment` model (`packages/prisma/schema.prisma`), `apps/api/src/comments/` module (controller/service, Webhooks-module-shaped), REST `GET/POST /services/pathfinder/v1/factSheets/{id}/comments`, GraphQL `createComment` mutation + `comments: [Comment!]!` field added to `BaseFactSheet` and all 9 (now 12) concrete types.

**Phase 2 — To-Dos.** New `Todo` model, `apps/api/src/todos/` module, REST `/services/todo/v1` (create/list/get/update/complete). REST-only, no GraphQL surface — matches real LeanIX's To-Do API being a separate REST microservice, not part of Pathfinder.

**Phase 3 — `TechCategory` + `Objective` fact sheet types.** No migration needed — fact sheet *types* are data-driven (`FactSheetType` table), not a TS/Postgres enum. Added to `DEFAULT_FACT_SHEET_TYPES` (`packages/shared/src/constants/default-meta-model.ts`), `CONCRETE_FACT_SHEET_TYPES` (`apps/api/src/graphql/resolvers/base-fact-sheet.fields.ts`), and matching SDL type blocks. `TechCategory` carries one extra field (`standardStatus`) — deliberately using the existing generic-attribute mechanism rather than inventing a "standards" subsystem, per the spec's "don't be smarter than LeanIX" rule. 4 new relation types wire both into the existing graph (`relApplicationToTechCategory`, `relITComponentToTechCategory`, `relProjectToObjective`, `relBusinessCapabilityToObjective`).

**Phase 4 — `AIAgent` fact sheet type + AI Agent Discovery endpoint.** Same data-driven-type pattern as Phase 3, plus `POST /services/aiagent/v1/discovery` (`apps/api/src/ai-agent-discovery/`) — accepts an agent-card-shaped payload and upserts it as an `AIAgent` fact sheet by calling `FactSheetService.create()` + `FactSheetPatchService.update()` directly (no parallel persistence path). Real A2A agent-card schema not independently confirmed (no license) — documented as best-effort in the DTO, same honesty convention as `upsertRelation`'s argument names.

**Phase 5 — Surveys.** The largest new subsystem: `SurveyDefinition`/`SurveyRun`/`SurveyInvitation`/`SurveyResponse` models, `apps/api/src/surveys/` module, REST `/services/survey/v1/{definitions,runs,runs/{id}/invitations,runs/{id}/responses,runs/{id}/results}`. Found and fixed a data-hygiene bug here: `listInvitations` was returning the raw `User` row (including `apiToken`/`apiTokenSecret`, both null for this seeded user but not always) instead of a narrow `{id, name, email}` projection — fixed before it shipped, unit test added to guard the `select` clause.

**Phase 6 — Multi-workspace support.** By far the largest phase — confirmed via research before starting that only `User`/`Webhook` had any `workspaceId` column; the entire meta-model/fact-sheet graph was implicitly single-tenant. Scoped down from full multi-tenancy to what M&A Architecture Assessment actually needs (documented, deliberate cut): `workspaceId` added to `FactSheetType`, `RelationType`, `FactSheet`, `TagGroup` (composite unique constraints: `[workspaceId, technicalKey]` etc.), left `User`/`Webhook`/`IntegrationConfiguration`/`SyncRun` as-is. Every read/write path in `FactSheetService`, `FactSheetPatchService`, `MetaModelService`, and the 4 GraphQL resolver methods that were missing `@CurrentUser()` (`factSheet`, `allFactSheets`, `deleteFactSheet`, `deleteRelation`) now threads the authenticated actor's `workspaceId`. `WebhookService`'s hardcoded `WORKSPACE_ID` constant removed in favor of the caller's actual workspace. `LdifProcessor` deliberately stays pinned to `ws-development` (documented cut — LDIF connectors are configured per-workspace in real LeanIX too, and this mock has no per-sync-run workspace selection). Seed script (`packages/prisma/seed.ts`) now seeds a second workspace (`ws-acquired-co`) with its own independent meta model and a small portfolio that overlaps *by name* (not id) with `ws-development`'s — e.g. "SAP CRM" vs. "SAP Customer 360" — which is what an M&A Architecture Assessment agent actually needs to detect.

Live-verified: two separately-authenticated tokens (`dev-token-12345` / `dev-token-acquired-11111`) see two different Application portfolios; a workspace-2 token reading a workspace-1 fact sheet by known id gets `null`; a cross-workspace `upsertRelation` is rejected with `FACT_SHEET_NOT_FOUND`; each workspace has its own `FactSheetType` row (different ids) for the same `technicalKey`; webhook registration/lookup is workspace-scoped. New `apps/api/test/e2e/multi-workspace.e2e-spec.ts` covers all of this.

**Two infrastructure bugs found and fixed along the way (not part of the plan):**

1. **Global `ValidationPipe({ whitelist: true })` was silently stripping every REST request body.** This repo's deliberate house style is undecorated, Swagger-only DTOs with hand-written service-layer validation (no `class-validator` anywhere) — `whitelist: true` strips every property from a plain object with zero validation decorators, which is exactly what all of them are. This broke the *already-shipped* webhook registration endpoint too (confirmed live: `identifier`/`targetUrl` came back "required" even when sent) — meaning it had been silently broken since whatever point this pipe option was added, undetected because nothing had re-exercised it since. Fixed in `apps/api/src/main.ts` by dropping `whitelist`, keeping `transform: true`.
2. **`prisma migrate dev`/`deploy` against this project's only `DATABASE_URL` (Neon's PgBouncer-pooled endpoint, `pgbouncer=true`) applies DDL but never durably records it in `_prisma_migrations`.** Discovered mid multi-workspace migration when `prisma migrate status` reported all 6 migrations (including 5 already-live-in-schema ones from earlier phases) as unapplied. Prisma's documented guidance is that migration commands need a direct, non-pooled connection; this project's `.env` only configures the pooled one. Worked around this session by resolving history against Neon's direct hostname (`prisma migrate resolve --applied <name>` per migration, then `migrate deploy` for the real new one) — also hit, and had to clear via `pg_terminate_backend`, a stuck `pg_advisory_lock` left by an earlier interrupted attempt (same root cause: PgBouncer transaction-pooling doesn't cleanly tear down session-level Postgres state on client disconnect). **Fixed durably same day**: added `directUrl = env("DIRECT_DATABASE_URL")` to `schema.prisma`'s datasource block (Prisma's built-in mechanism for exactly this — migration commands use `directUrl` automatically, the running app still uses the pooled `url`), `DIRECT_DATABASE_URL` documented in `.env`/`.env.example`/`.env.mock` and added to `render.yaml` (`sync: false`, filled in via Render dashboard). Verified: `prisma migrate status` with zero manual overrides resolves to the non-pooled hostname and reports the schema up to date.

Full suite after Phase 6: 55 unit + 22 e2e (16 pre-existing + 6 new multi-workspace), all green.

## 6. Next Steps (for a future session)

Everything in spec sections 1-17 (Phases 1-3) is implemented and live-verified; Phase 4 is done except the two items in §4 above. Tier-B use cases (§5 above) are closed. If picking this back up:
...
4. Tier-C use cases (external-system-gated — TCO, Cyber Impact, Licensing, etc.) remain open; see `LeanIX_Mock_UseCase_Coverage_Analysis.md` §3 for the scoping decision that needs making before starting any of them (mock external systems in this repo vs. a sibling repo vs. leave to whoever builds the agents).

## 7. Remote MCP endpoint — matching real LeanIX's actual contract (2026-09-01)

`apps/mcp` previously only spoke stdio — useless for any AI Studio/agent-builder platform expecting a remote MCP server. Research this session (SAP's own `github.com/SAP/leanix-ai-plugins/blob/main/MCP-SETUP.md`, `leanix.net/en/blog/mcp-server-for-sap-leanix-solutions`) found that real LeanIX's MCP server has been generally available since November 2025 as a remote Streamable HTTP endpoint — correcting the wrong claim struck through in §4c above. Built to match:

- **New route**, mounted inside `apps/api` (not a new service — reuses the existing deployment/TLS/`AuthService`): `POST /services/mcp-server/v1/mcp?toolsets=inventory`, matching real LeanIX's path exactly. `apps/api/src/mcp-server/mcp-server.controller.ts` (`@Public()`, bypasses the global `LeanIxAuthGuard` since this endpoint parses a different auth contract itself), `mcp-server-auth.service.ts`, `mcp-server.module.ts`.
- **Auth** resolved per-request, in-process (no self-HTTP round trip for the auth check): `Authorization: Token <api-token>` → `AuthService.validateApiToken()` + `issueToken()` directly via Nest DI; `Authorization: Bearer <jwt>` → `JwtService.verify()`, falling back to the Token path if it's not a valid JWT (mock leniency). `AuthModule` now re-exports its `JwtModule` registration so this new module can inject `JwtService` without a second `registerAsync()`.
- **Tool implementations reused as-is**, not rewritten: `apps/mcp/src/tools/*.ts` were already transport-agnostic (take an `McpServer` + `LeanIxClient`, don't know about stdio vs. HTTP). Added a barrel `apps/mcp/src/index.ts` (deliberately *not* re-exporting `server.ts`, which has a side-effecting `main()` that starts a stdio server on import), flipped `apps/mcp`'s `declaration` on in `tsconfig.json` so `apps/api` can import it as a typed workspace dependency (`"@leanix-mock/mcp": "*"`, same pattern as `@leanix-mock/shared`), and pointed `package.json`'s `"main"` at the new `dist/index.js` (safe — the `start`/`start:dev` scripts already hardcode `dist/server.js`/`src/server.ts` directly, not `main`).
- **Data calls** still go through `LeanIxClient`'s existing GraphQL/REST HTTP calls, just pointed at `http://localhost:{config.port}` (loopback) instead of the public `LEANIX_BASE_URL` — a small addition to `LeanIxClient` (`presetToken` config field) makes it use the already-resolved JWT directly instead of doing its own OAuth exchange.
- **Toolsets**: only one exists (`inventory`, the 8 existing tools). `?toolsets=` omitted defaults to `inventory` (mock-only leniency — real LeanIX returns nothing if omitted); any other value returns zero tools. Verified live: a real MCP `Client`+`StreamableHTTPClientTransport` against `?toolsets=automations` gets a `Method not found` on `tools/list` — an SDK quirk when zero tools are ever registered (the `McpServer` never wires the list-tools handler), not a bug, and not worth working around for a toolset this mock doesn't implement anyway.
- **Stateless**: `sessionIdGenerator: undefined`, fresh `McpServer`+transport per POST, matching the SDK's own documented stateless pattern (`node_modules/@modelcontextprotocol/sdk/dist/esm/examples/server/simpleStatelessStreamableHttp.js`) — no sticky sessions needed on Render, and naturally supports different callers presenting different workspace credentials on the same deployed endpoint.

**Live-verified** (real `Client`+`StreamableHTTPClientTransport`, not just unit tests): `tools/list` + `search_fact_sheets` + `get_meta_model` over `Token` auth and over a separately-obtained `Bearer` JWT; a `ws-acquired-co` token correctly gets `ws-acquired-co`'s portfolio through the identical endpoint a `ws-development` token hits; bad token and missing-header both cleanly 401 with a JSON-RPC error body, not a crash; GET/DELETE both 405. The pre-existing stdio path re-verified unchanged after the `index.ts`/`package.json` changes.

New tests: `apps/api/test/unit/mcp-server-auth.service.spec.ts` (6 tests — Token/Bearer/fallback/missing/invalid), `apps/api/test/e2e/mcp-server.e2e-spec.ts` (6 tests — real MCP client against a real ephemeral port, both auth schemes, both rejection paths, 405s, cross-workspace scoping). Full suite: 61 unit + 28 e2e, all green.

**Documentation corrected**: `docs/API_REFERENCE.md`'s MCP section now documents both transports (remote HTTP with the real URL/auth/toolsets shape, local stdio as the mock-only convenience); its "what's mock-only" list gained 3 new entries for the leniencies above; §4c above is struck through with the correction inline rather than silently deleted.
