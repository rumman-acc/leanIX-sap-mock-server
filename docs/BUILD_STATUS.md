...
1. Make the direct (non-pooled) migration connection durable — add `DIRECT_DATABASE_URL` to `.env`/`.env.example` and document `DATABASE_URL="$DIRECT_DATABASE_URL" npx prisma migrate ...` as the required invocation, so the next migration doesn't rediscover §5's infra finding #2 the hard way.
2. If Docker is available: run `docker compose -f docker/docker-compose.yml up` and confirm the containerized stack matches the natively-verified behavior above; fix any container-specific issues (paths, env propagation).
3. Optional: build a minimal Admin UI (React, per spec) if still desired.
4. Optional: add a load/perf test pass if the consuming application needs specific throughput guarantees.
5. Tier-C use cases (external-system-gated — TCO, Cyber Impact, Licensing, etc.) remain open; see `LeanIX_Mock_UseCase_Coverage_Analysis.md` §3 for the scoping decision that needs making before starting any of them (mock external systems in this repo vs. a sibling repo vs. leave to whoever builds the agents).
