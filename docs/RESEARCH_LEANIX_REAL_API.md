...
## 4. Smaller, more contained corrections found

- **`updateFactSheet` patch paths for real fields differ from what this mock accepts**, per directly-quoted real examples:
  - Lifecycle: `{"op":"replace","path":"/lifecycle/phaseIn","value":"2022-07-01"}` — real LeanIX patches **one phase at a time** by its phase name in the path (`/lifecycle/plan`, `/lifecycle/phaseIn`, `/lifecycle/active`, ...), not this mock's single `/lifecycle` replace-the-whole-object approach.
  - `externalId`: `{"op":"replace","path":"/externalId","value":"{\"type\":\"ExternalId\",\"externalId\":\"123456789\"}"}` — real `externalId` is a **structured object** (`{type, externalId}`, supporting multiple external-ID "types"/source systems per fact sheet), not the plain string this mock stores.
  - `qualitySeal` is directly patchable: `{"op":"replace","path":"/qualitySeal","value":"approve"}` — note **lowercase** `"approve"`, not this mock's uppercase `APPROVED` enum value. This mock never exposes a patch path for `qualitySeal` at all today (it's only ever auto-set to `BROKEN`).
  - A real response example includes a `lxState` field on `factSheet { id name lxState }` — worth confirming whether this is what this mock calls `status`, or an additional/different concept (not confirmed either way — **LOW confidence**, flagging for follow-up rather than acting on it).
